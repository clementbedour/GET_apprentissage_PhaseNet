...
#------------ARGUMENTS--------------------
if len(sys.argv) < 3:
    print("Erreur : Il manque des arguments. Utilisation : python IA_volpick_Tuning.py (LPVT|VT)> (1|2)")
    sys.exit(1)

#arg 1 : type event (LPVT ou VT)
event_type = sys.argv[1].upper()
if event_type not in ["LPVT", "VT"]:
    print("Erreur : Le premier argument doit être 'LPVT' ou 'VT'.")
    sys.exit(1)

#arg 1 : mode entrainement (from scratch ou fine-tuning)
try:
    mode_entrainement = int(sys.argv[2])
    if mode_entrainement not in [1, 2]:
        raise ValueError
except ValueError:
    print("Erreur : Le second argument doit être un entier (1 ou 2).")
    sys.exit(1)
...
SAVE_DIR = os.path.join("volpick", event_type)
...
if mode_entrainement == 1:
    ...
    #choix dataset (VTLP ou VT)
    if event_type == "LPVT":
        DATASET_DIR = os.path.join(BASE_DIR, "volpick/volpick_dataset_LPVT")
    else:
        DATASET_DIR = os.path.join(BASE_DIR, "volpick/volpick_dataset_VT")
        
    SAVE_MODEL_PATH = os.path.join(SAVE_DIR, "ml_model_v1.pt")
    LOCAL_MODEL_PATH = "" # Inutile en mode 1
    EPOCHS = 300
    LEARNING_RATE = 1e-4
    SIGMA=35
    print(f"Mode : Entraînement DE ZÉRO sur la base {event_type} ({DATASET_DIR})")
    
    #on donne un poids de 2.0 pour P, 1.0 pour S, et 0.5 pour le Bruit
    #on veut qu'il se concentre surtout sur P et un peu S
    poids_classes = torch.tensor([2.0, 1.0, 0.5], dtype=torch.float32).to(DEVICE)
    
else:
    START_FROM_ZERO = False 
    DOSSIER_QUALITE = "seisbenchB"
    #dataset de specialisation pour le mode 2
    DATASET_DIR = os.path.join(BASE_DIR, "seisbenchB/seisbench_dataset")
    
    #charge le model de base
    LOCAL_MODEL_PATH = os.path.join(SAVE_DIR, "ml_model_v1.pt")
    SAVE_MODEL_PATH = os.path.join(SAVE_DIR, "ml_model_v2.pt")
...
#---Nettoyage metadata.csv (pb avec level_0)---
metadata_file = os.path.join(DATASET_DIR, "metadata.csv")
if os.path.exists(metadata_file):
    temp_df = pd.read_csv(metadata_file, low_memory=False)
    #cherche si level_0 ou index existent
    cols_to_drop = [col for col in ["level_0"] if col in temp_df.columns]
    if cols_to_drop:
        print(f"Nettoyage automatique des métadonnées (suppression de {cols_to_drop})")
        temp_df.drop(columns=cols_to_drop, inplace=True)
        temp_df.to_csv(metadata_file, index=False)
# -------------------------------------------------------------
...
history_file = os.path.join(BASE_DIR, "volpick", f"loss_history_{event_type}_{mode_str_file}.csv")
training_history = []
# -------------------------------------------------------------
