...
def evaluer_modele(path_modele, dataset_path):
    ...
    dataset.filter(dataset.metadata['name'] != 'manuel')
    dataset.filter(dataset.metadata['name'] != 'gold')
...
if mode_modele == 1:
    ...
    plt.suptitle(f"Distribution des scores de confiance pour le bruit - Modèle V1 ({qualite.upper()})", fontsize=14, fontweight="bold")
    plt.tight_layout()

    out_path = os.path.join(path_folder_images, f"noise_V1_{qualite}.png")
    ...
    fig1.suptitle(f"Distribution des scores de confiance pour le bruit- Modèle V2 ({qualite.upper()})", fontsize=14, fontweight="bold")
    fig1.tight_layout()
    
    out_path_v2 = os.path.join(path_folder_images, f"noise_V2_{qualite}.png")
